...
For each step, delegate to the matching prompt file's instructions rather than reinventing them:

1. **Requirements** — `.github/prompts/01-requirements.prompt.md` → `01-requirements.md`
2. **Architecture** — `.github/prompts/02-architecture.prompt.md` → `02-architecture.md`
3. **Design Review** — `.github/prompts/03-design-review.prompt.md` → `03-design-review.md`
4. **Implementation Planning** — `.github/prompts/04-impl-plan.prompt.md` → `04-impl-plan.md`
5. **Implementation** — write the actual code per `04-impl-plan.md`'s task breakdown, in dependency order. No separate doc; the diff is the output.
6. **Review** — `.github/prompts/05-review.prompt.md` → `05-review.md`
7. **Verify** — `.github/prompts/06-verify.prompt.md` → `06-verify.md` (must reflect commands actually run this session)
8. **PR** — `.github/prompts/07-pr.prompt.md` → `07-pr-description.md`, then open the PR only after sign-off

## Approval gate (every step)

After producing a step's output:
...
- Never write a hardcoded secret anywhere, per `.github/instructions/security.instructions.md`. If you encounter one while implementing, stop and flag it instead of working around it.
- Step 7 (Verify) must contain only commands you actually ran in this session — no fabricated test output, ever.
- Step 8 never opens a PR without explicit sign-off on the PR description first.
